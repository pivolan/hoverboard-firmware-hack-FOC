...
// ############################ VARIANT_HOVERCAR SETTINGS ############################
  ...
  #undef  CTRL_MOD_REQ
  #define CTRL_MOD_REQ            SPD_MODE  // SPD mode: speed control via PI regulator. No active braking (modified).
  #define CONTROL_ADC             0         // use ADC as input. Number indicates priority for dual-input. Disable CONTROL_SERIAL_USART2, FEEDBACK_SERIAL_USART2, DEBUG_SERIAL_USART2!
  ...
  #define ADC_ALTERNATE_CONNECT               // use to swap ADC inputs (input1 <-> input2)
